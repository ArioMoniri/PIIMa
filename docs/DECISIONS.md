...
> **D-042 and D-046 below were written, lost, and restored.** During the same session that created
> them they were destroyed by a `git checkout --` run against uncommitted files to undo a cosmetic
> change. The code artifacts survived because they are separate files; these two entries did not.
> They are reconstructed from what still cites them (`README.md`, `docs/L2-CANDIDATES.md`,
> `eval/label_maps/modernbert-tr-pii-ner.yaml`), so the decisions are faithful while the original
> wording is not recoverable. Recorded here rather than restored silently, because an append-only
> log whose gaps are invisible is not an append-only log.
>
> The numbering gap at D-043, D-044 and D-045 is real and deliberate: entries were written at those
> ids in the same session and nothing that survived cites them, so their content cannot be honestly
> reconstructed. The ids stay burned rather than reused.

## D-042 — `ytu-ce-cosmos/modernbert-tr-pii-ner` is the first L2 candidate, and a candidate until it is measured

- **Date:** 2026-07-20
- **Status:** ACCEPTED. Wired, unmeasured.

**Decision.** Integrate `ytu-ce-cosmos/modernbert-tr-pii-ner` as the first candidate for the L2
ensemble. Apache-2.0, 149.4M parameters, 25 KVKK-oriented entity types over 51 BIO labels, a cased
WordPiece base of 50,008 tokens, ONNX and safetensors, built on `ytu-ce-cosmos/modernbert-tr-base`.
It is the first Turkish PII checkpoint we have found that is cased, permissively licensed, and
labelled along KVKK lines, and it is exactly the thing this project has been blocked on.

**Alternatives.** Continue with no L2 and keep reporting 0.0000 name recall. Fine-tune our own,
which needs annotated Turkish clinical text under a DUA that we do not have. Use a general
multilingual NER model, which has neither the Turkish-specific identifier classes nor a cased
Turkish base.

**Rationale.** The label set maps onto our schema unusually well: `KISI_AD_SOYAD` covers the name
classes that are the entire L2 hole, `TUZEL_KISI` covers facilities, and `TCKN`, `VKN`, `IBAN_TR`,
`TELEFON`, `EMAIL`, `ADRES` and `DOGUM_TARIHI` overlap L1, which finally gives the union something
to agree with and makes multi-detector agreement a real state rather than a code path with no
callers.

**Consequences, including the ones that cost us.**

- **Our published numbers become dependent on somebody else's checkpoint.** Under I5 a model card
  may only quote a committed eval run; from here a run is not identified by its `eval_sha` alone.
  The checkpoint AND its revision must be recorded, because a name without a revision is not
  provenance. That binding is what `docs/L2-CANDIDATES.md` and the label map refer to.
- **We should expect it to fail our gates, and that is not a defect in it.** It reports 80.85
  overlap micro-F2 on a 300-document test set of Turkish web PDFs, high-court decisions and
  synthetic documents. Our corpus is clinical, our floor is 0.98 recall, and F2 on their corpus is
  not recall on ours. Quoting their number as ours would be the precise error
  `docs/COMPARISON.md` section 1.1 criticises in someone else's work.
- **Partial name masking is a harder disclosure problem than none.** A build that masks no names is
  easy to describe. A build that masks most names invites the assumption that it masks all of them,
  and that assumption is the one with a patient on the other end of it.
- The int8 quantization drops to 71.90 on their own benchmark. For a recall-critical task under I2
  that is not a free optimisation, so fp32 is the default and int8 is an explicit opt-in.

## D-046 — A third-party label set is mapped as DATA, and `MESLEK_UNVAN` is mapped but NOT F1-scored

- **Date:** 2026-07-20
- **Status:** ACCEPTED. Renumbered from a colliding D-042 (see the note above).

**Decision.** Map a third-party label set to our schema in a YAML file under `eval/label_maps/`,
never in code, so a second checkpoint with a different label set needs a new file and no code change.
Every source label resolves to a schema id or to an explicit null; an unknown label is a hard error;
a schema id claimed by two source labels requires a stated `shared_target_reason` and the loader
rejects an unexplained collision.

**The `MESLEK_UNVAN` decision, which is the reason this ADR exists.** `MESLEK_UNVAN` is
occupation and title. Our schema files that as `EMPLOYER_ROLE`, a class B quasi-identifier which
D-008 says is validated by the L6 re-identification red team and deliberately NOT scored by F1,
because narrative re-identification has no clean ground truth. A third-party model emitting a
token-level prediction for it is a genuinely new situation that D-008 did not anticipate.

It is mapped so the prediction is visible, and it is NOT added to the F1-scored set. Quietly
promoting it would have reversed D-008 by accident, and the loader now refuses `scored: true` on any
quasi target so that promoting it requires editing the loader rather than a line of YAML. This ADR
records the proposal and leaves the current gate untouched.

**`SAGLIK_BILGISI` is mapped to nothing, on purpose.** It is health information, PHI under KVKK, and
we have no direct equivalent. Masking all health information would destroy the clinical note, which
is exactly what the class C medical allowlist exists to prevent. The collision between "PHI under
KVKK" and "the vocabulary that must never be masked" is real and needs a human decision, so it is
flagged rather than resolved by default.

**Consequences.** `KISI_AD_SOYAD` is one label against our three name roles, so a match policy has
to decide whether one prediction may satisfy `PATIENT_NAME`, `CLINICIAN_NAME` or `RELATIVE_NAME`
without counting once per role. That policy is specified and NOT yet implemented in the harness,
which is stated in `docs/L2-CANDIDATES.md` rather than left for someone to discover from a number
that looks too good.
